...
PhylogeneticsVersion[]:=Print["Phylogenetics for Mathematica 6.5\n(c) P. David Polly, 28 Sept 2019\n"];
...
(*  This function estimates Pagel's Lambda using the Likelihood method described by Freckleton et 
	al. 2002. It uses to functions, the first that calulates the probability of the traits given
	V and lambda, and the second which maximizes this function for the traits.  

	Usage:  mylineage = PagelsLambda[tree, trait];

	where tree is a tree in Newick format or a tree table and trait is a vector trait values
	Created by P. David Polly,  24 September 2014

*)


Ptrait[mytrait_,V_,\[Lambda]_]:=Block[{MakeLambda,LambdaV,anc,ratesq,p},
MakeLambda=Table[Table[If[i==j,1,\[Lambda]],{i,Length[V]}],{j,Length[V]}];
LambdaV=V*MakeLambda;
anc=(Table[1,{Length[mytrait]}].Inverse[LambdaV].mytrait)/(Table[1,{Length[mytrait]}].Inverse[LambdaV].Table[1,{Length[mytrait]}]);
ratesq=1/(Length[mytrait]-1)*((mytrait-anc*Table[1,{Length[mytrait]}]).Inverse[LambdaV].(mytrait-anc*Table[1,{Length[mytrait]}]));
p=(1/((2*Pi*ratesq)^(Length[mytrait]/2))*(Norm[LambdaV]^(-0.5))*Exp[(-1/(2*ratesq))*((mytrait-anc*Table[1,{Length[mytrait]}]).Inverse[LambdaV].(mytrait-anc*Table[1,{Length[mytrait]}]))]);
p];

PagelsLambda[tree_,trait_]:=Block[{treeTable,lambda,V,l,ancFinal,ratesqFinal,MakeLambdaFinal,LambdaVFinal,ancAdjusted,ratesqAdjusted,mytrait},
...
V=PhylogeneticMatrices[treeTable][[1,2;;,2;;]];

If[Length[Dimensions[trait]]!= 2, Return["Trait variable must have two columns"]];
If[Sort[trait[[1;;,1]]]!= Sort[Union[Select[treeTable,#[[4]]==1&][[1;;,1]]]], Return["Trait labels must equal tree names"]];
mytrait=trait[[1;;,2]];

lambda=NMinimize[{0-Ptrait[mytrait,V,l],l>=0&&l<=1.2},l][[2,1,2]];

ancFinal=(Table[1,{Length[mytrait]}].Inverse[V].mytrait)/(Table[1,{Length[mytrait]}].Inverse[V].Table[1,{Length[mytrait]}]);
ratesqFinal=1/(Length[mytrait]-1)*((mytrait-ancFinal*Table[1,{Length[mytrait]}]).Inverse[V].(mytrait-ancFinal*Table[1,{Length[mytrait]}]));

MakeLambdaFinal=Table[Table[If[i==j,1,lambda],{i,Length[V]}],{j,Length[V]}];
LambdaVFinal=V*MakeLambdaFinal;
ancAdjusted=(Table[1,{Length[mytrait]}].Inverse[LambdaVFinal].mytrait)/(Table[1,{Length[mytrait]}].Inverse[LambdaVFinal].Table[1,{Length[mytrait]}]);
ratesqAdjusted=1/(Length[mytrait]-1)*((mytrait-ancAdjusted*Table[1,{Length[mytrait]}]).Inverse[LambdaVFinal].(mytrait-ancAdjusted*Table[1,{Length[mytrait]}]));

{"\[Lambda]"->lambda,"AncestorReconstruction"->ancFinal,"\[Sigma]2"->ratesqFinal,"adjusted AncestorReconstruction"-> ancAdjusted,"adjusted \[Sigma]2"->ratesqAdjusted}
...
(* This function transforms a phylogenetic covariance matrix (the C matrix of authors, VarY of Martins and Hansen) into a Newick format tree.
	...
	The first input parameter contains a phylogenetic covariance matrix in the sme format as produced in element 1 (VarY) by the PhylogeneticMatrices[] 
