...
PhylogeneticsVersion[]:=Print["Phylogenetics for Mathematica 7.0\n(c) P. David Polly, 20 April 2025\n"];
...
(* This function draws a Newick format tree and labels the nodes and tips, making use of helpful suggestions given by Felsenstein (2004).
    ...
    7 April 2025: Updated to add user-controlled supression of node labels.

*)

DrawNewickTree[tree_,nodelabels_:1,aspect_:0]:=Block[
{TreeTable,myTable,ExtNodeNames,ImmediateDescendants,FindLineage,TipEntries,NodeEntries,NodeNames,TipNames,TipY,TipLineages,NodeLineages,Descendants,NodeY,TreeX,TreeY,TreeXY,x,TreeLines,TreeTipLabels,TreeNodeLabels,TreeBaseLabels,FullTree},
...
(* node labels *)
TreeNodeLabels=Table[{FontFamily->"Arial",FontSize->10,Text[NodeNames[[x]],Flatten[TreeXY[[Flatten[Position[TreeXY[[1;;,1]],NodeNames[[x]]]],{2,3}]]],{-1.3,0}]},{x,Length[NodeNames]}];
(* Base node and label *)
TreeBaseLabels={{PointSize[0.01], Point[Flatten[TreeXY[[Flatten[Position[TreeXY[[1;;,1]],"Node 0"]],{2,3}]]]]}, Text["Node 0",Flatten[TreeXY[[Flatten[Position[TreeXY[[1;;,1]],"Node 0"]],{2,3}]]],{-1.3,0}]};

If[nodelabels==1,
FullTree={TreeLines,TreeTipLabels,TreeNodeLabels,TreeBaseLabels},
FullTree={TreeLines,TreeTipLabels}
];

If[aspect==0,
Return[Graphics[FullTree,Axes->{True,False},AxesOrigin->{0,0*Length[TipNames]},AspectRatio->Log[Length[TipNames]]/GoldenRatio]],
Return[Graphics[FullTree,Axes->{True,False},AxesOrigin->{0,0*Length[TipNames]},AspectRatio->aspect]]
]
...
(*  This function estimates Adams's (2014)  multivariate Kappa from tree and univariate trait vector.
	  ...
	  Created by P. David Polly, 21 December 2018
	  Capped leaky variable "v" that was accidentally set globally instead of trapped locally.  

  *)

KappaMultivariate[tree_,traits_]:=Block[{varY,varAY,varA,Cmat,TipLabels,y,Imat,Ivec,a,EofY,Dya,eigvects,eigvals,Emat,Uy,PDuo,num, denom, kappamult,v},
{varY,varAY, varA}=PhylogeneticMatrices[tree];
Cmat=varY[[2;;,2;;]];
TipLabels=varY[[2;;,1]];
...
(*	This function maps discrete characters onto a phylogenetic tree by tracing states down from tips and representing ancestral states
		as probabilities based on the states of descendant tips weighted by the duration of branch lengths from node to those tips.
	  
	  Usage:  MapDiscreteTraitsOntoTree[MyTree, MyTrait ("StateColors"-> MyColors)]
	  
	  where MyTree is a Newick-format tree in the format of ReadNewick[], MyTrait
	  is a matrix of taxon names and trait values where the names match exactly the taxon 
	  names in MyTree. MyColors is an optional matrix with the state names in the first column 
	  and color values in the second column.
	  
	  Created by P. David Polly, 20 April 2025
	  *)

MapDiscreteTraitsOntoTree[tree_,traits_,OptionsPattern[{"StateColors"->{"Default"},"NodeLabels"->False}]]:=
Block[{i,j,x,Descendants,ExtNodeNames,FindLineage,ImmediateDescendants,Legend,LegendFactor,myarc, mylen,mylin,mylineages,
mynodeancestors,mynodestates,mystates,myTable,nodearcs,NodeColors,nodecounts,NodeEntries,NodeLineages,NodeNames,
nodestateproportions,NodeY,thisstate,TipColors,TipEntries,TipLabels,TipLineages,TipNames,TipTrans,TipY,TraitColors,
TraitValues,TreeLines,TreeTable,TreeX,TreeXY,TreeY},
...
TraitValues=Union[traits[[1;;,2]]];
If[Flatten[OptionValue["StateColors"]][[1]]!="Default"&&Sort[TraitValues]!=Sort[OptionValue["StateColors"][[1;;,1]]],Return["States in color data do not match trait data."],""];

If[Flatten[OptionValue["StateColors"]][[1]]=="Default",
mystates=Union[traits[[1;;,2]]];TraitColors=Table[mystates[[x]]->ColorData[97,x],{x,Length[mystates]}],
TraitColors=#[[1]]->#[[2]]&/@OptionValue["StateColors"]
];

TipTrans=#[[1]]->#[[2]]&/@(traits/.TraitColors);
...
TreeLines={{Thick,Line[{Flatten[TreeXY[[Flatten[Position[TreeXY[[1;;,1]],#[[1]]]],{2,3}]]],Flatten[{Flatten[TreeXY[[Flatten[Position[TreeXY[[1;;,1]],#[[2]]]],{2}]]],Flatten[TreeXY[[Flatten[Position[TreeXY[[1;;,1]],#[[1]]]],{3}]]]}]}]&/@TreeTable},{Thick,Line[{Flatten[{Flatten[TreeXY[[Flatten[Position[TreeXY[[1;;,1]],#[[2]]]],{2}]]],Flatten[TreeXY[[Flatten[Position[TreeXY[[1;;,1]],#[[1]]]],{3}]]]}],Flatten[TreeXY[[Flatten[Position[TreeXY[[1;;,1]],#[[2]]]],{2,3}]]]}]&/@TreeTable}};
(* tip labels *)
TipLabels = Table[{Text[StringReplace[TipNames[[x]],{"_"->" ", "-"->" "}],Flatten[TreeXY[[Flatten[Position[TreeXY[[1;;,1]],TipNames[[x]]]],{2,3}]]]+{1+GoldenRatio/Log[Length[traits]],0},{-1,0}]},{x,Length[TipNames]}];

(* tip colors *)
TipColors = Table[{TipNames[[x]]/.TipTrans,Rectangle[Flatten[TreeXY[[Flatten[Position[TreeXY[[1;;,1]],TipNames[[x]]]],{2,3}]]]+{0.5,-0.4},Flatten[TreeXY[[Flatten[Position[TreeXY[[1;;,1]],TipNames[[x]]]],{2,3}]]]+{0.5+0.9*GoldenRatio/Log[Length[traits]+1],0.4}]},{x,Length[TipNames]}];

(* node colors *)
mylineages=Table[mylin=GetFullLineage[traits[[i,1]],TreeTable];
mylen=0;Table[mylen=mylin[[j,3]]+mylen;{mylin[[j,2]],traits[[i,2]],mylen},{j,Length[mylin]}],{i,Length[traits]}];
Do[mylineages[[x,1;;,3]]=mylineages[[x,1;;,3]]/Plus@@mylineages[[x,1;;,3]],{x,Length[mylineages]}];
nodecounts=GatherBy[Sort[Flatten[mylineages,1]],#[[1]]&];
Do[nodecounts[[x,1;;,3]]=nodecounts[[x,1;;,3]]/Plus@@nodecounts[[x,1;;,3]],{x,Length[nodecounts]}];
nodestateproportions=Table[mynodestates=Union[nodecounts[[x,1;;,2]]];
mynodeancestors={nodecounts[[x,1,1]],Table[thisstate=Select[nodecounts[[x]],#[[2]]==mynodestates[[i]]&];{mynodestates[[i]],Plus@@thisstate[[1;;,3]]},{i,Length[mynodestates]}]},{x,Length[nodecounts]}];
nodearcs=Table[myarc=0;{nodestateproportions[[x,1]],Table[{nodestateproportions[[x,2,i,1]],myarc,myarc=myarc+nodestateproportions[[x,2,i,2]]},{i,Length[nodestateproportions[[x,2]]]}]},{x,Length[nodestateproportions]}];
NodeColors=Table[Table[{nodearcs[[x,2,i,1]]/.TraitColors,Disk[Select[TreeXY,#[[1]]==nodearcs[[x,1]]&][[1,{2,3}]],{.3,0.6},(360*# Degree&/@nodearcs[[x,2,i,{2,3}]])-90Degree]},{i,Length[nodearcs[[x,2]]]}],{x,Length[nodearcs]}];

(* Legend 
If[Max[TreeXY[[1;;,3]]]<10,LegendFactor=1,LegendFactor=2];
Legend={{CapForm["Butt"],AbsoluteThickness[6],ColorLine[MinTrait,MaxTrait,{{0,Max[TreeXY[[1;;,3]]]+LegendFactor},{Max[TreeXY[[1;;,2]]]/2,Max[TreeXY[[1;;,3]]]+LegendFactor}},ancvalue,MaxTrait,MinTrait,OptionValue["ColorPattern"]]},
Text[ToString[NumberForm[MinTrait,{3,2}]],{0,Max[TreeXY[[1;;,3]]]+LegendFactor},{-1,-2}],
Text[ToString[NumberForm[MaxTrait,{3,2}]],{Max[TreeXY[[1;;,2]]]/2,Max[TreeXY[[1;;,3]]]+LegendFactor},{1,-2}],
Text[ToString[NumberForm[ancvalue,{3,2}]],{((ancvalue-MinTrait)/(MaxTrait-MinTrait))*Max[TreeXY[[1;;,2]]]/2,Max[TreeXY[[1;;,3]]]+LegendFactor},{0,-2}]};
*)
